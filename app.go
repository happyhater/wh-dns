...
// run initializes configuration and runs the WhiteHat DNS
...
func run(args options) {
	...
	log.Printf("WhiteHat DNS, version %s\n", VersionString)
	log.Debug("Current working directory is %s", config.ourWorkingDir)
	if args.runningAsService {
		log.Info("WhiteHat DNS is running as a service")
	}
	...
	if config.firstRun {
		log.Info("This is the first launch, redirecting everything to /install.html ")
...
func requireAdminRights() {
	...
	if runtime.GOOS == "windows" {
		log.Fatal("This is the first launch. You must run it as Administrator.")

	} else {
		log.Error("This is the first launch. You must run it as root.")

		_, _ = io.WriteString(os.Stdout, "Do you want to start as root user? [y/n] ")
...
func printHTTPAddresses(proto string) {
	...
	if proto == "https" && config.TLS.ServerName != "" {
		...
		log.Println("WhiteHat DNS is available on the following addresses:")
